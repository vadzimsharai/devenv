    ...
    opts = function(_, opts)
        ...
        opts.dashboard = {
            ...
            sections = {
                ...
                {section = "keys", gap = 1, padding = 3}, {section = "startup"}
            }
        }
        ...
        opts.indent = {
            ...
            filter = function(bufnr)
                return buf_utils.is_valid(bufnr) and
                           not buf_utils.is_large(bufnr) and vim.g.snacks_indent ~=
                           false and vim.b[bufnr].snacks_indent ~= false
            end,
    ...
    specs = {
        {
            ...
            opts = function(_, opts)
                ...
                local snack_opts =
                    require("astrocore").plugin_opts "snacks.nvim"
                ...
                if vim.tbl_get(snack_opts, "gitbrowse", "enabled") ~= false then
                    if vim.fn.executable "git" == 1 then
                        maps.n["<Leader>g"] =
                            vim.tbl_get(opts, "_map_sections", "g")
                        maps.n["<Leader>go"] = {
                            function()
                ...
                if vim.tbl_get(snack_opts, "picker", "enabled") ~= false then
                    maps.n["<Leader>f"] =
                        vim.tbl_get(opts, "_map_sections", "f")
                    if vim.fn.executable "git" == 1 then
                        maps.n["<Leader>g"] =
                            vim.tbl_get(opts, "_map_sections", "g")
                        maps.n["<Leader>gb"] = {
                    ...
                    maps.n["<Leader>ff"] = {
                        function()
                            require("snacks").picker.files {
                                hidden = vim.tbl_get(
                                    (vim.uv or vim.loop).fs_stat ".git" or {},
                                    "type") == "directory"
                            }
                        end,
                        desc = "Find files"
            ...
            opts = {
                commands = {
                    find_files_in_dir = function(state)
                        ...
                        local path = node.type == "file" and
                                         node:get_parent_id() or node:get_id()
                ...
                filesystem = {
                    window = {
                        mappings = {
                            ...
                            fw = vim.fn.executable "rg" == 1 and
                                "find_words_in_dir" or nil,
                            fW = vim.fn.executable "rg" == 1 and
                                "find_all_words_in_dir" or nil
                        }
                    }
                }
            }
